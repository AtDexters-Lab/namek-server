...
2. **Device filesystem must survive the DB wipe.** The entire recovery model assumes device-side state (`/var/lib/piccolod/`) is intact. If a correlated event wipes both server DB and device filesystems (infrastructure compromise, fleet-wide OS update resetting state), recovery fails and devices become new identities. This is the fundamental assumption of device-driven recovery.
