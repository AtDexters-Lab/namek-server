...
func BootstrapZone(ctx context.Context, client *PowerDNSClient, dnsCfg config.DNSConfig, logger *slog.Logger) error {
	backoff := time.Second
	deadline := time.Now().Add(60 * time.Second)

	for {
		err := tryBootstrap(ctx, client, dnsCfg, logger)
		if err == nil {
			return nil
		}
...
func tryBootstrap(ctx context.Context, client *PowerDNSClient, dnsCfg config.DNSConfig, logger *slog.Logger) error {
	...
	logger.Info("creating dns zone", "zone", dnsCfg.Zone)
	return client.CreateZone(ctx, dnsCfg.Zone, dnsCfg.BaseDomain, dnsCfg.RelayHostname)
}
